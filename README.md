...
- Web library defaults to Movies (standalone ≥50m); `?len=clip` / `?len=all`; search, genre filter (`?genre=`), thumbnails (TMDB poster when known), and play counts
