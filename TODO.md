...
- [x] **Unified title pipeline** — drop `--tmdb-titles` / `--llm-titles`; TMDB → LLM → PHP after every link; `--no-tmdb` / `--no-llm` / `--force`; full rebuild: `--series-backfill --force`.
...
- [x] **Cron** — incremental `php scan.php` already enriches title gaps when TMDB/LLM are configured; `--series-backfill --force` for full rebuild; `--no-tmdb` / `--no-llm` for tests.
